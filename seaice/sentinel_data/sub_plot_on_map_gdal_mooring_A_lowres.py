...
def sub_plot_on_map_gdal(path, file,lon_mooring,lat_mooring):
    ...
    m = Basemap(projection='tmerc', lat_0=lat_mooring, lon_0=lon_mooring, \
                llcrnrlon=15.,llcrnrlat=80.,urcrnrlon=25.,urcrnrlat=82, resolution='f')
    ...
    # annotate
    x_text,y_text = m(15,81.9)
    plt.annotate(date, xy=(x_text,y_text),color='red')
    
    #m.drawcountries()
    m.drawcoastlines(linewidth=.5, color='yellow')
    m.drawlsmask(land_color="0.8")

    plt.savefig(path + date, dpi=600, bbox_inches='tight', pad_inches=0)
    plt.cla()