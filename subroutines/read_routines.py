...
def add_coll_vars(data_dict):
    ...
    for name in data_dict:
        ...
        ds['smap_iqc_flag'] = xr.DataArray(np.zeros(ilen, dtype='int32'), coords={'time': ds.time}, dims=('time'))
...
def add_coll_vars_ds_jpl(ds):
    ...
    ilen = ds.time.shape[0]
    ds['deltaT'] = xr.DataArray(np.ones(ilen, dtype='float32')*99999, coords={'time': ds.time}, dims=('time'))
    ds['smap_SSS'] = xr.DataArray(np.ones(ilen, dtype='float32')*np.nan, coords={'time': ds.time}, dims=('time'))
    ds['smap_fland'] = xr.DataArray(np.ones(ilen, dtype='float32'), coords={'time': ds.time}, dims=('time'))
    ds['smap_fice'] = xr.DataArray(np.zeros(ilen, dtype='float32'), coords={'time': ds.time}, dims=('time'))
    ds['smap_iqc_flag'] = xr.DataArray(np.zeros(ilen, dtype='int32'), coords={'time': ds.time}, dims=('time'))
#    ds['smap_iqc_flag'] = xr.DataArray(np.empty(ilen, dtype='uint16'), coords={'time': ds.time}, dims=('time'))
    ds['smap_name'] = xr.DataArray(np.empty(ilen, dtype='U125'), coords={'time': ds.time}, dims=('time'))
    ds['smap_dist'] = xr.DataArray(np.ones(ilen, dtype='float32')*99999, coords={'time': ds.time}, dims=('time'))
    ds['smap_ydim'] = xr.DataArray(np.zeros(ilen, dtype='float32'), coords={'time': ds.time}, dims=('time'))
    ds['smap_xdim'] = xr.DataArray(np.zeros(ilen, dtype='float32'), coords={'time': ds.time}, dims=('time'))
    ds['smap_rev_number'] = xr.DataArray(np.zeros(ilen, dtype='int32'), coords={'time': ds.time}, dims=('time'))
    return ds
...
def add_coll_vars_ds_rss(ds):
    ...
    ds['deltaT'] = xr.DataArray(np.ones(ilen, dtype='float32')*99999, coords={'time': ds.time}, dims=('time'))
    ds['smap_SSS'] = xr.DataArray(np.ones(ilen, dtype='float32')*np.nan, coords={'time': ds.time}, dims=('time'))
    ds['smap_SSS_40km'] = xr.DataArray(np.ones(ilen, dtype='float32')*np.nan, coords={'time': ds.time}, dims=('time'))
    ds['smap_fland'] = xr.DataArray(np.zeros(ilen, dtype='float32'), coords={'time': ds.time}, dims=('time'))
    ds['smap_fice'] = xr.DataArray(np.zeros(ilen, dtype='float32'), coords={'time': ds.time}, dims=('time'))
    ds['smap_iqc_flag'] = xr.DataArray(np.zeros(ilen, dtype='int32'), coords={'time': ds.time}, dims=('time'))
    ds['smap_name'] = xr.DataArray(np.empty(ilen, dtype='U125'), coords={'time': ds.time}, dims=('time'))
    ds['smap_dist'] = xr.DataArray(np.ones(ilen, dtype='float32')*99999, coords={'time': ds.time}, dims=('time'))
